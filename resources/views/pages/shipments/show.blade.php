                ...
                <div class="col-10">
                    ...
                    <div class="d-flex justify-content-between mt-4">
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <button id="emailInvoiceBtn" type="button" class="btn btn-outline-primary mr-2"> Email invoice</button>
...
<button type="button" id="emailInvoiceBtn" class="btn btn-outline-primary">Email invoice</button>

<script>
    $(document).ready(function() {
        $('#emailInvoiceBtn').click(function() {
            var $btn = $(this);
            $btn.prop('disabled', true).text('Sending...');
            const email = @json($shipment['clientEmail']);
            const clientId = @json($shipment['clientId']);
            const clientName = @json($shipment['clientName']);
            const code = @json($shipment['code']);

            $.ajax({
                url: '{{ route("pages.shipments.sendinvoice") }}',
                method: 'GET',
                data: {
                    id: clientId,
                    email: email,
                ...
                error: function(xhr, status, error) {
                    alert(xhr.responseJSON?.message || 'Failed to send invoice.');
                },
                complete: function() {
                    $btn.prop('disabled', false).text('Email invoice');
                }
            });
        });
    });
