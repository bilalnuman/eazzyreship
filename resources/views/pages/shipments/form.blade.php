  ...
  $(function () {
    ...
    function renderPreview() {
      ...
      shipperDocs.forEach(function (file, idx) {
        all.push({ file: file, idx: idx, kind: 'attach' });
      });
      capturedPhotos.forEach(function (file, idx) {
        all.push({ file: file, idx: idx, kind: 'photo' });
      });

      all.forEach(function (item) {
        var file = item.file;

        var $wrap = $('<div/>', { class: 'doc-item' });

        // small label (optional)
        $wrap.append(
          $('<small/>', { text: (item.kind === 'photo' ? 'Photo' : 'File') })
        );
        ...
        if (file.type && file.type.indexOf('image/') === 0) {
          ...
          $wrap.append($('<div/>', { class: 'pdf-box', text: 'PDF' }));

        } else {
          $wrap.append($('<div/>', {
            text: file.name || 'File',
            css: { width: '110px', wordBreak: 'break-word' }
          }));
        }
    ...
    function openNativeInput(opts) {
      ...
      var $temp = $('<input/>', { type: 'file', multiple: multiple });
    ...
    async function captureToFile() {
      ...
      return new File([blob], 'captured-' + Date.now() + '.jpg', { type: 'image/jpeg' });
    }
    ...
    $takePhotoBtn.on('click', async function () {
      ...
      if (isMobile()) {
        openNativeInput({ capture: true, multiple: true, target: 'photo' });
        return;
      }
    ...
    $attachBtn.on('click', function () {
      openNativeInput({ capture: false, multiple: true, target: 'attach' });
    });
