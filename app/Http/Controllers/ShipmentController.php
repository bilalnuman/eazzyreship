...
class ShipmentController extends Controller
{
    ...
    public function store(Request $request, $token = null)
    {
        ...
        $validated = $request->validate([
            ...
            // file arrays (validated here, but NOT inserted into shipments table)
            'attachments_before_shipping.*' => 'nullable|file|mimes:jpg,jpeg,png,pdf|max:4096',
            'capturedImages.*'              => 'nullable|file|mimes:jpg,jpeg,png|max:4096',

            // keep if you still use base64 carrier doc
            'captured_image' => 'nullable|string',
        ]);

        // ----------------------------
        // IMPORTANT: Remove file keys BEFORE create()
        // ----------------------------
        unset($validated['attachments_before_shipping'], $validated['capturedImages'], $validated['captured_image']);
        ...
        try {
            ...
            // ----------------------------
            if ($token) {
                $order = (string) $request->input('order_id', '');
                $parts = explode('-', $order);
            ...
            // Store packages
            // ----------------------------
            if ($request->filled('packages') && is_array($request->packages)) {
            ...
            // ----------------------------
            // Store attachments_before_shipping (files)
            // ----------------------------
            if ($request->hasFile('attachments_before_shipping')) {
                ...
                foreach ($request->file('attachments_before_shipping') as $file) {
                    ...
                    try {
                        // store file in S3 under attachments/
            ...
            // ----------------------------
            // Store capturedImages (files) ✅ FIXED
            // ----------------------------
            ...
            // ----------------------------
            // OPTIONAL: base64 captured_image -> carrier_doc
            // ----------------------------
            ...
            return redirect()->back()
                ->with('message', 'An error occurred while creating the Shipment. ' . $e->getMessage())
                ->with('icon', 'error')
                ->withInput();
        }
    }
